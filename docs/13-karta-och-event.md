...
Verkliga event hämtas nu via `/api/events` från Københavns Biblioteker. Ett källänkat Luma-event finns som manuellt kontrollerat exempel. [Källor och drift](15-eventkallor.md) beskriver täckning och uppdatering. Veckolistan visar kommande event och hoppar till rätt tid vid val. Detaljpanelen visar omslag, arrangör, plats, program när det finns och extern anmälan.
...
Synlighetsregeln är `startsAt <= vald tid < endsAt`. Ett event som slutar 17:00 försvinner exakt 17:00. Ett event som pågår över midnatt får synas på båda datumen, men bara inom intervallet. Ogiltiga intervall visas inte. Alla och Event visar tidsmässigt giltiga event; Bar, Mat och Touchgrass visar sina respektive platser. Sökningen gäller eventnamn, plats och taggar. Sol och OSM-öppettider används inte för att gissa om ett event pågår.
...
Demo-event är avstängda från start och aktiveras med **Visa demo-event** i filtren. Dessa event är påhittade och erbjuds inte som bokningsbara evenemang. Tre exempel skapas för den aktuella sjudagarsvyn: måndagshäng, livemusik nästa dag och utomhusbio på lördag. De finns för att prova kartan och tidsövergångarna. Fel i den verkliga eventkällan ersätts aldrig automatiskt med demo-event.
